...
> 이 저장소는 [제품 쇼케이스 사이트 템플릿](https://github.com/sangji90-dotcom/showcase-template)을
> **출판사 자사 도서 사이트**로 구성한 영업용 데모입니다.
> 물결서가와 등장하는 책·지은이·ISBN은 전부 가상이며 실재하지 않습니다.
>
> 서점(교보·알라딘 등)과 달리 **장바구니·결제·회원이 없습니다.**
> 구매는 상세 페이지의 "구매처" 링크로 외부 판매처에 넘깁니다.
> 데모라서 그 링크는 예시 전용 도메인(example.com)으로 걸어 두었습니다.
>
> 도서 50권과 표지는 `npm run gen:books` 로 생성됩니다.
> 데이터는 `scripts/books-data.mjs` 에 있습니다.
